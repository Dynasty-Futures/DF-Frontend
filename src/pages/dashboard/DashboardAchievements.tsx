...
import { 
  Award, Target, CircleDollarSign, TrendingUp, Zap, Gem, Crown, Lock, Medal, Percent, Trophy,
  Snowflake, Timer, Layers, Shield, CalendarCheck, Globe, Diamond, BookOpen, Repeat
} from 'lucide-react';
...
const rarityConfig: Record<Rarity, { label: string; className: string; glowClass: string }> = {
  common: {
    label: 'Common',
...
const isRecentlyUnlocked = (dateStr?: string): boolean => {
  ...
  const now = new Date();
  const diffDays = Math.floor((now.getTime() - unlockDate.getTime()) / (1000 * 60 * 60 * 24));
  return diffDays <= 7;
};
...
const achievements: Achievement[] = [
  ...
  },
  {
    ...
    description: 'Recover from max drawdown to break even',
    icon: <TrendingUp size={24} className="text-primary" />,
    category: 'performance',
    rarity: 'epic',
    state: 'locked',
...
const RarityTag = ({ rarity, muted = false }: { rarity: Rarity; muted?: boolean }) => {
...
const AchievementCard = ({ achievement, index }: AchievementCardProps) => {
  const { title, description, icon, rarity, state, unlockedDate, progress } = achievement;
  const config = rarityConfig[rarity];
  ...
  const hasShimmer = isUnlocked && (rarity === 'epic' || rarity === 'legendary');
  ...
  return (
    <Tooltip delayDuration={200}>
        ...
        >
          ...
          <div className="relative flex items-start gap-4">
            ...
            <div className={cn(
              ...
              isUnlocked && "bg-primary/15 border-primary/30 animate-icon-3d-float",
            ...
            <div className="flex-1 min-w-0 pr-16">
              ...
              {isUnlocked && unlockedDate && (
                <div className="mt-3 inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-primary/10 border border-primary/20">
                  ...
                  <span className="text-xs text-primary font-medium">Unlocked {unlockedDate}</span>
                </div>
              )}

              {/* Progress bar for in-progress */}
              {isInProgress && progress && (
                <ProgressBar current={progress.current} target={progress.target} label={progress.label} />
              )}
            </div>
          </div>
        </div>
      ...
      <TooltipContent side="top" className="max-w-xs p-4 space-y-2">
        ...
        {isInProgress && progress && (
          <p className="text-xs text-primary font-medium">Progress: {progress.label}</p>
        )}
        {isUnlocked && unlockedDate && (
          <p className="text-xs text-primary font-medium">Unlocked: {unlockedDate}</p>
        )}
        {isLocked && (
          <p className="text-xs text-muted-foreground italic">Complete the requirements to unlock</p>
        )}
        <div className="pt-1">
          <RarityTag rarity={rarity} muted={isLocked} />
        </div>
...
const CategorySection = ({ title, achievements }: { title: string; achievements: Achievement[] }) => {
...
const DashboardAchievements = () => {
  ...
  const payoutAchievements = achievements.filter(a => a.category === 'payout');
    ...
    <div className="space-y-8 pt-16 lg:pt-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Track your trading milestones and unlock rewards</p>
        </div>
        <div className="flex items-center gap-3 px-5 py-3 rounded-2xl bg-primary/10 border border-primary/30">
          <Award size={24} className="text-primary" />
          <span className="text-xl font-bold text-foreground">{unlockedCount}</span>
      ...
      <div className="space-y-10">
        <CategorySection title="Progression" achievements={progressionAchievements} />
        <CategorySection title="Performance" achievements={performanceAchievements} />
        <CategorySection title="Payout & Profit" achievements={payoutAchievements} />
      </div>
    </div>
  );
};

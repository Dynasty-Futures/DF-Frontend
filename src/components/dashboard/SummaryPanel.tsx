import { Target, TrendingDown, TrendingUp, Calendar, Zap, ArrowUpRight, ArrowDownRight, Flame, SnowflakeIcon, ChevronLeft, ChevronRight } from 'lucide-react';
...
const SummaryItem = ({ label, value, icon, variant = 'default' }: SummaryItemProps) => {
  const variantStyles = {
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <div className={cn(
          ...
          <span className="text-xs font-semibold text-foreground whitespace-nowrap flex-shrink-0">{value}</span>
        </div>
      </TooltipTrigger>
...
const SummaryPanel = ({ accountId, selectedDate, onDateChange }: SummaryPanelProps) => {
  ...
  const profitProgress = Math.min(100, Math.max(0, (account.closedPnL / account.profitTarget) * 100));
  ...
  return (
    ...
    <div className="p-5 rounded-2xl bg-card/50 backdrop-blur-sm border border-border/30 h-full">
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-base font-semibold text-foreground">Evaluation Progress</h3>
      ...
      <div className="mb-5 p-3 rounded-lg bg-primary/5 border border-primary/20">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-xs font-semibold text-primary">
            {formatCurrency(account.closedPnL)} / {formatCurrency(account.profitTarget)}
          </span>
        </div>
      ...
      <div className="mb-5">
        <span className="text-[10px] text-muted-foreground uppercase tracking-wider font-medium">Funding Requirements</span>
